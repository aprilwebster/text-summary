...
class TextSummary {
  ...
  constructor(options, profile) {
    ...
    // Select localization dictionary and profile parser based on the version of PI, V2 or V3
    if (this._version === 'v3'){
      this._i18n = new I18nDataV3(this._options.locale);
      this._profile = new PersonalityProfileV3(profile);
    } else{
      this._i18n = new I18nDataV2(this._options.locale);
      this._profile = new PersonalityProfileV2(profile);
    }

    //DEBUGGING PROFILE
    console.log('version is ' + this._version);
    console.log('locale is ' + this._options.locale);
    console.log('traits are:');
    this._profile.traits().forEach(function (t){
  ...
  //getSummary() {
    ...
    //return this.assemble(this._profile.tree).map(function (paragraph) { return paragraph.join(' '); }).join('\n');
