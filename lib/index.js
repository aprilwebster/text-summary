...
class TextSummary {
  ...
  constructor(options) {
    ...
    //this._personalityProfile = new PersonalityProfileV2();
  ...
  getSummary(profile) {
    //console.log('DEBUG: getSummary with profile ' + JSON.stringify(profile,2,null));
  ...
  assemble(tree) {
    ...
    traits.children.forEach(function (t) {
      console.log(t.name);
    ...
    console.log('NEEDS');
    needs.children.forEach(function (n) {
      console.log(n.name);
    });
    console.log('VALUES');
    values.children.forEach(function (v) {
      console.log(v.name);
    });


    return [
      //this.assembleTraits(tree.children[0]),
      //this.assembleFacets(tree.children[0]),
